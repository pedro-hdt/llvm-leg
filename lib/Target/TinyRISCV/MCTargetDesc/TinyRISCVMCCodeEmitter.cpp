//===-- TinyRISCV/LEGMCCodeEmitter.cpp - Convert LEG code to machine code -------===//
//
...
public:
  TinyRISCVMCCodeEmitter(const MCInstrInfo &mcii, MCContext &ctx)
      : MCII(mcii), CTX(ctx) {}

  ~TinyRISCVMCCodeEmitter() {}
...
MCCodeEmitter *llvm::createTinyRISCVMCCodeEmitter(const MCInstrInfo &MCII,
                                            ...
                                            MCContext &Ctx) {
  return new TinyRISCVMCCodeEmitter(MCII, Ctx);
}

/// getMachineOpValue - Return binary encoding of operand. If the machine
/// operand requires relocation, record the relocation and return zero.
unsigned TinyRISCVMCCodeEmitter::getMachineOpValue(const MCInst &MI,
                                             const MCOperand &MO,
                                             SmallVectorImpl<MCFixup> &Fixups,
                                             const MCSubtargetInfo &STI) const {
  if (MO.isReg()) {
  ...
  case MCSymbolRefExpr::VK_TINYRISCV_LO: {
    FixupKind = TinyRISCV::fixup_leg_mov_lo16_pcrel;
    break;
  }
  case MCSymbolRefExpr::VK_TINYRISCV_HI: {
    FixupKind = TinyRISCV::fixup_leg_mov_hi16_pcrel;
    break;
  }
  }
...
unsigned TinyRISCVMCCodeEmitter::getMemSrcValue(const MCInst &MI, unsigned OpIdx,
                                          SmallVectorImpl<MCFixup> &Fixups,
                                          const MCSubtargetInfo &STI) const {
  unsigned Bits = 0;
