//===-- TinyRISCVISelLowering.cpp - TinyRISCV DAG Lowering Implementation ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the TinyRISCVTargetLowering class.
//
//===----------------------------------------------------------------------===//
...
const char *TinyRISCVTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  default:
    return NULL;
  case TinyRISCVISD::RET_FLAG: return "RetFlag";
  case TinyRISCVISD::LOAD_SYM: return "LOAD_SYM";
  case TinyRISCVISD::CALL:     return "CALL";
  }
}

TinyRISCVTargetLowering::TinyRISCVTargetLowering(TinyRISCVTargetMachine &TinyRISCVTM)
    : TargetLowering(TinyRISCVTM), Subtarget(*TinyRISCVTM.getSubtargetImpl()) {
  ...
  setStackPointerRegisterToSaveRestore(TinyRISCV::X2);
...
SDValue TinyRISCVTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
...
SDValue TinyRISCVTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG& DAG) const
{
  EVT VT = Op.getValueType();
  GlobalAddressSDNode *GlobalAddr = cast<GlobalAddressSDNode>(Op.getNode());
  SDValue TargetAddr =
      DAG.getTargetGlobalAddress(GlobalAddr->getGlobal(), Op, MVT::i32);
  return DAG.getNode(TinyRISCVISD::LOAD_SYM, Op, VT, TargetAddr);
}
...
/// TinyRISCV call implementation
SDValue TinyRISCVTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_TinyRISCV);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    SDValue StackPtr = DAG.getRegister(TinyRISCV::X2, MVT::i32);
  ...
  Chain = DAG.getNode(TinyRISCVISD::CALL, Loc, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
SDValue TinyRISCVTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_TinyRISCV);
...
/// TinyRISCV formal arguments implementation
SDValue TinyRISCVTargetLowering::LowerFormalArguments(
  ...
  CCInfo.AnalyzeFormalArguments(Ins, CC_TinyRISCV);
  ...
  for (auto &VA : ArgLocs) {
    if (VA.isRegLoc()) {
      ...
      const unsigned VReg = RegInfo.createVirtualRegister(&TinyRISCV::GPRRegClass);
...
bool TinyRISCVTargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool isVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, isVarArg, MF, RVLocs, Context);
  if (!CCInfo.CheckReturn(Outs, RetCC_TinyRISCV)) {
    return false;
  }
...
SDValue
TinyRISCVTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool isVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_TinyRISCV);
  ...
  return DAG.getNode(TinyRISCVISD::RET_FLAG, dl, MVT::Other, RetOps);
}
